...
*NOTE*: STRs are traditionally defined as repeating motifs that are between 1 to 6bp long. For this truth set we exclude 1bp (homopolymer) repeats since they are uniquely error-prone, but include motifs longer than 6bp so that downstream users can choose which ones to include in their analyses.
