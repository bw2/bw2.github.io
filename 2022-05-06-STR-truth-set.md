...
HipSTR shows poor accuracy largely because it refuses to call many loci, instead outputing the following error message: "Aborting genotyping of the locus as the sequence upstream/downstream of the repeat is too repetitive for accurate genotyping". If we exclude these loci for all tools, this discards 30,394 out of 139,244 (21.8%) of truth set loci that have 2 to 6bp motifs (meaning if we use HipSTR, we are guaranteed to miss those variants), and the plot then looks like:
