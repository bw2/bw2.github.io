...
ExpansionHunterDenovo [<a href="https://genomebiology.biomedcentral.com/articles/10.1186/s13059-020-02017-z">Dolzhenko 2020</a>] is a widely used tool that is different from  ExpansionHunter, GangSTR and most other STR callers in several important ways. First, like SNV and SV callers, it looks for STR expansions genome-wide, without requiring the user to pre-specify which loci it should call. However, it can only detect loci where the total length of the repeat sequence in the reference genome + the STR variant exceeds 150bp (or whatever the short read length is in the input sample). Additionally, for the loci it does detect, it only outputs a rough estimate of expansion size and approximate genomic coordinates (+/- several hundred base pairs). This data from its initial per-sample "profile" step is then combined across samples and used for outlier tests that check for unusually large expansion(s) in each sample relative to other samples in the cohort. Here we use the truth set to see how well the "profile" step detects large expansions in CHM1-CHM13-2. 

We can also look at the distribution of loci that ExpansionHunterDenovo's "profile" step picks up for inclusion in subsequent outlier tests even though there is no pure STR expansion anywhere near there according to the truth set. These loci aren't false positives yet, but have the potential to become false positives if the outlier tests find sufficient variation in read counts across samples at these loci. 

There are two significant issues with this analysis. First, while the truth set (for now) only includes pure repeats, ExpansionHunterDenovo allows interruptions in the repeat sequence. When detecting which reads are repetative, it uses a heuristic algorithm that requires at least 85% of a read's sequence to be consistant with a pure repeat of some motif. This means that a subset of the expansions that ExpansionHunterDenovo detects are not in the truth set because they contain interruptions. Second, the "profile" step doesn't distinguish between loci where the sample differs from the reference from loci that simply have a long (longer than 150bp) stretch of repeats in the reference genome. 
