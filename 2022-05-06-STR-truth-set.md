...
In addition to tool evaluations, we can use this truth set to explore several interesting questions about STRs in general:
...
Since users must specify the set of all loci to genotype, an important question for any genome-wide STR analysis is which STR catalog to genotype. 
Two popular options are: 
1. a catalog generated by running TandemRepeatFinder on the reference genome using either default or non-default parameters.
2. a catalog of STRs shown to be polymorphic in population controls. 

An important question we can ask using the truth set is - if genotype all loci in one of these catalogs, how many STR variants are we guaranteed to miss because they fall outside the catalog?
