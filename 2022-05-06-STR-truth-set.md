...
ExpansionHunterDenovo [<a href="https://genomebiology.biomedcentral.com/articles/10.1186/s13059-020-02017-z">Dolzhenko 2020</a>] is a widely used tool that is different from  ExpansionHunter, GangSTR and most other STR callers in several important ways. First, like SNV and SV callers, it looks for STR expansions genome-wide, without requiring the user to pre-specify which loci it should call. However, it can only detect expansions if they are larger than the Illumina short read length - typically 150bp. Additionally, for the expansions it detects, it only outputs a rough estimate of expansion size and approximate genomic coordinates (+/- several hundred base pairs of the true locus). This data from its initial per-sample "profile" step is then combined across samples and used for outlier tests that check for unusually large expansion(s) in each sample relative to other samples in the cohort. We can use the truth set to see how well this step detects the large expansions in CHM1-CHM13-2. 
...
One significant issue with this analysis is that, while the truth set (for now) only includes pure repeats, ExpansionHunterDenovo allows interruptions in the repeat sequence. When detecting which reads are repetative, it uses a heuristic algorithm that requires at least 85% of a read's sequence to be consistant with a pure repeat of some motif. This means that a subset of the loci it detects will not be in the truth set because they are not pure expansions. 
