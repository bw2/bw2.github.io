...
Yes - through additional prefiltering. For the tests above, I prefiltered each individual's variants to exclude those that are common in the general popualation (keeping only variants with gnomAD v3 PopMax AF < 0.01). Applying this prefilter didn't change the number of true positive results, but improved their average rank from 7.2 to 5.6 (when considering the 62 out of 75 cases where the correct result was among the top 50 genes).
